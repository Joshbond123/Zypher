#!/usr/bin/env python3
"""zypher_agent.py — alias: same as hermes_sidecar.py for Supabase bridge sidecar."""
...
def main():
    ...
    while True:
        n+=1
        if n%HB==0:
            upsert("active_sessions",{"github_run_id":RUN_ID,"is_active":True,"instance_num":1,"updated_at":datetime.now(timezone.utc).isoformat()})
            ...
            try:
                ...
                for line in lines:
                    ...
                    if m["type"] in("telegram","response","tool","task"):
                        upsert("task_log",{"task_id":h[:16],"session_id":f"hermes-{RUN_ID}","status":m["type"],"message":m["content"][:2000],"tool_name":m["type"],"created_at":datetime.now(timezone.utc).isoformat()})
