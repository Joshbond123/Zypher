#!/usr/bin/env python3
"""hermes_key_rotator.py — Round-robin Cerebras key rotation (supports up to 5 keys)."""
import os,json,sys
from urllib.request import urlopen,Request

KEYS=[k for k in[os.environ.get(f"CEREBRAS_API_KEY{'' if i==0 else f'_{i+1}'}","") for i in range(5)] if k.strip()]
...
def select():
    ...
    print("WARN: all health checks failed (possible IP restriction) — using key 0");return KEYS[0]
